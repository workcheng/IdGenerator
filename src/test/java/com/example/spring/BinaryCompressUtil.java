...
public class BinaryCompressUtil {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 999; i++) {
            ...
            final Integer reduction = reduction(x);
            final String complete = complete(6, i);
            System.out.println(i + "," + x + "," + reduction  + "," + (i == reduction) + "," + complete+ "," + reduction(complete));
        }
    }

    @Test
    void test1() {
        final String format = String.format("%05d", "89");
...
//    private static final String UNIT_STR = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ@#$%^&*()_+={}[]:;<>";//82进制 四位表示数字最大值:45212175
    private static final String UNIT_STR = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";//82进制 四位表示数字最大值:45212175
    /**
     * 表示使用的是多少位进制
     */
